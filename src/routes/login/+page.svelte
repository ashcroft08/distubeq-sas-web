		...
		<div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl">
			...
			>
				...
				{#if data?.expired && !form?.message}
					<div
						in:fade
						class="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 p-3.5 text-sm font-semibold text-blue-700"
						role="status"
					>
						...
						<span>Su sesión ha expirado por inactividad. Ingrese sus credenciales nuevamente.</span>
					</div>
				{/if}

				<!-- Mensajes de error del formulario -->
				{#if form?.message}
					{@const isServer = form.errorType === 'server'}
					<div
						in:fade
						class="flex items-center gap-3 rounded-xl border p-3.5 text-sm font-semibold {isServer
							? 'border-amber-200 bg-amber-50 text-amber-700'
							: 'login-error-shake border-red-100 bg-red-50 text-red-600'}"
						role="alert"
					>
						{#if isServer}
							<!-- Ícono de advertencia para errores de servidor -->
							<svg class="h-5 w-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
								<path
									fill-rule="evenodd"
						...
						{:else}
							<!-- Ícono de error para credenciales -->
							<svg class="h-5 w-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
								<path
									fill-rule="evenodd"
...
<style>
	...
	/* Animación de shake sutil para errores de credenciales */
	.login-error-shake {
